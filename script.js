...
function handleFileSelect(event) {
    // Prevent multiple file selection triggers
    if (isFileSelecting) {
        return;
    }
    ...
    // This ensures the change event will fire even if the same file is selected
    event.target.value = '';

    // Reset the flag after a short delay to allow the event cycle to complete
    setTimeout(() => {
        isFileSelecting = false;
    }, 100);
}
...
function addFiles(files) {
    // Create a Set of existing file keys (name + size) to avoid duplicates
    const existingFileKeys = new Set();
    appState.files.forEach(f => {
        existingFileKeys.add(`${f.name}_${f.file.size}`);
    });
    ...
    const newFiles = files.map(file => ({
        ...
        const key = `${fileObj.name}_${fileObj.file.size}`;
        if (existingFileKeys.has(key)) {
            showError(`File "${fileObj.name}" is already added`);
            return false;
        }
